...
(function (window, document) {
    ...
    function activateElement(router, element, url, eventDetail) {
        ...
        if (router.previousRoute) {
            // fire('previous-route', {'path': router.previousRoute.attributes.path.value }, router);
            fire('previous-route', router.previousRoute, router);
            router.previousRoute.removeAttribute('active');
        }
        // fire('active-route', {'path': router.activeRoute.attributes.path.value }, router);
        fire('active-route', router.activeRoute, router);
        ...
        // add the new content
        if (!router.activeRoute.innerHTML.trim().length) {
    ...
    function deactivateRoute(route) {
        if (route) {
            ...
            while (node) {
                ...
                if (route.getAttribute('state') !== 'persist') {
